...
from app.modules.milestone.schemas import goal_type_for_milestone, validate_milestone_attributes
from app.modules.milestone.services.risk import assert_safe_milestone
from datetime import datetime, timezone, date, timedelta



def get_goal_for_user(session: Session, user_id: uuid) -> Milestone:
    goal = session.exec(select(Milestone).where(Milestone.created_for == user_id).where(Milestone.active==True)).first()
    if not goal:
        raise HTTPException(status_code=404, detail="Goal not found")
    return goal


def get_goals_for_user(session: Session, user_id: uuid.UUID) -> list[Milestone]:
    goals = session.exec(
        select(Milestone)
        .where(Milestone.created_for == user_id)
        .order_by(Milestone.created_at.desc())
    ).all()
    return list(goals)

def create_goal_for_user(session: Session, user_id: uuid.UUID, payload: Milestone, appointment_id: Optional[uuid.UUID] = None) -> Milestone:
    ...
    if g.active:
        ...
        # Deactivate others
        existing_active = session.exec(select(Milestone).where(Milestone.created_for == user_id).where(Milestone.active == True)).all()
...
def delete_goal_for_user(session: Session, user_id: uuid.UUID) -> None:
    ...
    goals = session.exec(select(Milestone).where(Milestone.created_for == user_id).where(Milestone.active == True)).all()
    for g in goals:
        g.active = False
        g.start_date=None
        g.end_date=None
...
def delete_goal_by_id(session: Session, user_id: uuid.UUID, goal_id: uuid.UUID) -> None:
    ...
    g = session.get(Milestone, goal_id)
    if not g or g.created_for != user_id:
        raise HTTPException(status_code=404, detail="Goal not found")
    session.exec(_delete(MilestoneLog).where(MilestoneLog.goal_id == goal_id))  # cascade
    session.delete(g)
    session.commit()


def activate_goal_for_user(session: Session, user_id: uuid.UUID, goal_id: uuid.UUID) -> Milestone:
    # 1. Verify ownership
    target_goal = session.get(Milestone, goal_id)
    if not target_goal:
         raise HTTPException(status_code=404, detail="Goal not found")
    ...
    existing = session.exec(select(Milestone).where(Milestone.created_for == user_id).where(Milestone.active == True)).all()
    for ex in existing:
        ex.active = False
        session.add(ex)
...
def add_goal_log(session: Session, user_id: uuid.UUID, payload: MilestoneLogCreate) -> MilestoneLog:
    # Get active goal
    goal = session.exec(
        select(Milestone)
        .where(Milestone.created_for == user_id)
        .where(Milestone.active == True)
    ).first()
...
def get_goal_logs(session: Session, user_id: uuid.UUID, goal_id: uuid.UUID) -> list[MilestoneLog]:
    # Ensure goal is owned by user
    goal = session.get(Milestone, goal_id)
    ...
    logs = session.exec(
        select(MilestoneLog)
        .where(MilestoneLog.goal_id == goal_id)
        .order_by(MilestoneLog.date.asc())
    ).all()
    return list(logs)
