...
from app.modules.daily_log.models import DailyLog
from app.modules.daily_goal.schemas import DailyGoalCreate, DailyGoalUpdate, validate_daily_goal_attributes
from app.modules.daily_log.schemas import DailyLogSubmit
from app.modules.daily_goal.services.risk import assert_safe_daily_goal
...
def create_daily_goal(session: Session, user_id: uuid.UUID, payload: DailyGoalCreate,
                      ...
                      plan_id: Optional[uuid.UUID] = None) -> DailyGoal:
    try:
    ...
    dg = DailyGoal(
        ...
        unit=payload.unit,          # enum — already constrained, never short()
        active=payload.active,
        days_of_week=payload.days_of_week,
        attributes=attrs,
        # Set up front rather than mutating + re-adding after the insert: assigning
        # plan_id post-commit meant a second add()/commit(), which SQLAlchemy flagged
        # with "Session.add() ... not supported within the execution stage of the flush".
        plan_id=plan_id,
    )
    # An unsafe goal is refused whether or not it is being activated now. Checking only
    # on activation let the setup chat write unsafe drafts freely (it always writes
    # active=False), and the whole plan then failed as a unit at activate_plan — long
    # after the conversation that could have corrected it had ended.
...
def update_daily_goal(session: Session, user_id: uuid.UUID, daily_goal_id: uuid.UUID,
    ...
    for f in ("name", "target_value", "unit", "milestone_id", "days_of_week"):
        if f in data:
            setattr(dg, f, short(data[f], 120) if f == "name" else data[f])
    ...
    # Checked whether or not it is active, same as create. `dg` is a persistent row we
    # have already mutated in place, so the rollback matters: without it the rejected
    # values stay dirty in the session and the next commit — from any later write in
    # the same request — flushes the very edit this refused.
    try:
        assert_safe_daily_goal(session, user_id, dg)
