...
from app.modules.appointment.models import FollowUpRoom, FollowUpMessage, TimeProposal, FollowUpRoomStatus, ProposalStatus
from app.modules.appointment.schemas import FollowUpRoomRead
...
def get_patient_summary(
    ...
    """Return patient UserData, Milestone (active or most recent), and NutritionTarget (active or most recent)."""
    ...
    goal = session.exec(
        select(Milestone)
        .where(Milestone.created_for == room.user_id)
        .where(Milestone.active == True)  # noqa: E712
    ).first()
    if not goal:
        goal = session.exec(
            select(Milestone)
    ...
    if goal:
        logs = session.exec(
            select(MilestoneLog)
            .where(MilestoneLog.goal_id == goal.id)
