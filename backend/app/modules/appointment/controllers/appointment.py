...
from app.modules.appointment.services.appointment import (
...
def get_consultant_history_controller(session: Session, consultant_profile_id: uuid.UUID, current_user: User) -> list[Appointment]:
    from app.modules.appointment.services.appointment import list_consultant_history
...
def get_user_history_controller(session: Session, user_id: uuid.UUID) -> list[Appointment]:
    from app.modules.appointment.services.appointment import list_user_history
...
def user_cancel_appointment(session: Session, me: User, appointment_id: uuid.UUID) -> Appointment:
    from app.modules.appointment.services.user_side_appointment import cancel_appointment
...
def get_appointment_details_controller(session: Session, me: User, appointment_id: uuid.UUID):
    from app.modules.appointment.services.appointment import get_appointment_details_with_extras
    from app.modules.user.schemas import UserRead, UserType
...
def toggle_permission_controller(session: Session, me: User, appointment_id: uuid.UUID, grant: bool) -> Appointment:
    from app.modules.appointment.services.appointment import toggle_appointment_permission
