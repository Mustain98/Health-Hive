...
# Actually, I'll check if I can interpret `GoalUpsert` from context. It's likely `MilestoneUpdate` or similar.
# `Milestone` has `MilestoneUpdate`. I'll use `MilestoneUpdate` or just keep `GoalUpsert` and let it break if it must. 
...
from app.modules.consultant.controllers.manage_user import (
    read_user_goal,
    create_user_goal,
    read_user_target,
    create_user_target,
...
@router.get("/users/{user_id}/goal", response_model=Milestone)
...
@router.post("/users/{user_id}/goal", response_model=Milestone)
